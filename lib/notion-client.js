...
class NotionClient {
  constructor() {
    ...
    // Cache configuration
    this.cacheDir = path.join(os.homedir(), '.notion-tui-cache');
    this.cacheFile = path.join(this.cacheDir, `tasks-${this.databaseId}.json`);
  ...
  async getUser(userId) {
    ...
    try {
      ...
      this.userCache[userId] = response;
      return response;
    } catch (error) {
  ...
  async getTasksByStatus(forceRefresh = false) {
    try {
      ...
      if (!forceRefresh) {
        ...
        if (cachedData) {
          ...
          cachedData._cacheAge = this.getCacheAge();
          return cachedData;
        }
      }
      ...
      this.saveToCache(tasksByStatus);
    ...
    try {
      ...
      const comments = await Promise.all(
        response.results.map(async (comment) => {
          const user = await this.getUser(comment.created_by.id);
          return {
            text: this.getRichText(comment.rich_text),
            author: user.name,
    ...
    switch (type) {
      ...
      case 'numbered_list_item':
        return `• ${this.getRichText(
          blockData.rich_text || blockData.text || []
        )}`;

      case 'to_do':
        const checked = blockData.checked ? '☑' : '☐';
        return `${checked} ${this.getRichText(
          blockData.rich_text || blockData.text || []
        )}`;
      ...
      for (const block of existingBlocks.results) {
        if (block.type !== 'unsupported') {
          try {
            ...
            console.error(
              `Failed to delete block ${block.id}: ${deleteError.message}`
            );
          }
        }
      }
  ...
  async createTask(title, status, content) {
    try {
      ...
      // Find the title property name by checking database schema
      ...
      if (!titlePropertyName) {
        return {
          success: false,
          message: 'Could not find title property in database'
        };
      }
      ...
      // Find and set the status property if it exists (could be 'select' or 'status' type)
      ...
      // Clear cache so the new task shows up
      this.clearCache();
    ...
    while (i < lines.length) {
      ...
      if (line.startsWith('# ')) {
        ...
        blocks.push({
          ...
          numbered_list_item: {
            rich_text: [
              { type: 'text', text: { content: line.replace(/^\d+\. /, '') } }
            ]
          }
        });
      } else if (line.startsWith('> ')) {
  ...
  getTitle(page) {
    const titleProperty =
      page.properties.Name || page.properties.Title || page.properties.Task;
    if (!titleProperty) {
  ...
  getAssignee(page) {
    ...
    if (
      ...
      return (
        assigneeProperty.people[0].name ||
        assigneeProperty.people[0].person?.email ||
        null
      );
    }
    return null;
  ...
  loadFromCache() {
    try {
      ...
      if (now - cacheData.timestamp > this.cacheExpiration) {
  ...
  saveToCache(tasksByStatus) {
    try {
      ...
      // If cache saving fails, just log it and continue
  ...
  getCacheAge() {
    try {
      ...
      const ageInSeconds = Math.floor(
        (Date.now() - cacheData.timestamp) / 1000
      );
