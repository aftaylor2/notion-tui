...
class NotionTUI {
  ...
  setupUI() {
    this.titleBox = this.grid.set(0, 0, 1, 12, blessed.box, {
      content: ' Notion Tasks Board - q:quit | r:refresh | /:search | Ctrl+N:new | ←→:columns | ↑↓:tasks | Enter:details | Ctrl+O:browser | Ctrl+E:edit ',
      style: {
        fg: 'white',
        bg: 'blue',
        bold: true
  ...
  async showTaskDetails() {
    ...
    const [content, comments] = await Promise.all([
      this.notionClient.getPageContent(task.id),
      this.notionClient.getComments(task.id)
    ]);
    ...
    if (comments && comments.length > 0) {
      detailContent.push('');
      detailContent.push('{yellow-fg}{bold}=== Comments ==={/}');
      detailContent.push('');
      comments.forEach(comment => {
        const commentDate = new Date(comment.created_time).toLocaleString();
        detailContent.push(`{cyan-fg}${comment.author} (${commentDate}):{/} ${comment.text}`);
      });
    }
  ...
  async createNewTask() {
    if (!this.selectedStatus) {
      this.updateStatus('Please select a status column first');
      return;
    }
    
    const currentStatus = this.selectedStatus;
    this.updateStatus(`Creating new task in ${currentStatus}...`);
    ...
    try {
      ...
      const tmpFile = path.join(os.tmpdir(), `notion-new-task-${Date.now()}.md`);
      
      // Prepare template content for new task
      let templateContent = `# New Task Title\n\n`;
      templateContent += `Status: ${currentStatus}\n`;
      templateContent += `---\n\n`;
      templateContent += `## Description\n\n`;
      templateContent += `Enter your task description here...\n\n`;
      templateContent += `## Details\n\n`;
      templateContent += `- Add any additional details\n`;
      templateContent += `- Or requirements\n`;
      templateContent += `- Or notes\n`;
      
      fs.writeFileSync(tmpFile, templateContent, 'utf-8');
      ...
      this.screen.program.pause();
      this.screen.program.normalBuffer();
      this.screen.program.hideCursor(false);
      process.stdin.setRawMode(false);
      process.stdin.pause();
      ...
      editorProcess.on('close', async (code) => {
        ...
        if (code === 0) {
          ...
          // Look for the title in the first heading
          ...
          // Remove the title and status lines from content
          const contentLines = [];
          let skipHeader = true;
          for (const line of lines) {
            if (skipHeader) {
          ...
          this.updateStatus(`Creating task: ${title}...`);
          
          // Create the task in Notion
          const result = await this.notionClient.createTask(title, currentStatus, content);
          
          if (result.success) {
            this.updateStatus(`Task created: ${title}`);
            ...
            this.updateStatus(`Failed to create task: ${result.message}`);
            this.screen.render();
          }
        } else {
          this.updateStatus('Task creation cancelled');
        }
      ...
      this.updateStatus(`Error creating task: ${error.message}`);
    }
  }
  ...
  updateTasks(tasksByStatus) {
    ...
    this.renderBoard();
    // Status will be set by the caller who knows if data is cached
  }
